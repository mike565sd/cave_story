...
namespace {
	...
	const units::Game kGunWidth = units::kHalfTile * 3;
	const units::Game kGunHeight = units::kHalfTile * 2;
...
void PolarStar::initializeSprites(Graphics& graphics) {
	...
	ENUM_FOREACH(h_facing, HORIZONTAL_FACING) {
		ENUM_FOREACH(v_facing, VERTICAL_FACING) {
				initializeSprite(graphics, boost::make_tuple(
					(HorizontalFacing)h_facing, (VerticalFacing)v_facing));
			}
		}
}
